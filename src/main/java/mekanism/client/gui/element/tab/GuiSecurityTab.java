...
public class GuiSecurityTab extends GuiTileEntityElement<TileEntity> {
    ...
    public Rectangle4i getBounds(int guiWidth, int guiHeight) {
        return new Rectangle4i(guiWidth + this.guiXSize, guiHeight + 32, 26, 26);
    }

    @Override
    protected boolean inBounds(int xAxis, int yAxis) {
        return xAxis >= this.guiXSize + 3 && xAxis <= this.guiXSize + 21 && yAxis >= 36 && yAxis <= 54;
    }

    @Override
    public void renderBackground(int xAxis, int yAxis, int guiWidth, int guiHeight) {
        mc.renderEngine.bindTexture(RESOURCE);
        guiObj.drawTexturedRect(guiWidth + this.guiXSize, guiHeight + 32, 0, 0, 26, 26);
        ...
        if (getOwner() != null && getOwner().equals(mc.player.getUniqueID()) && (data == null || !data.override)) {
            guiObj.drawTexturedRect(guiWidth + this.guiXSize + 3, guiHeight + 36, renderX, inBounds(xAxis, yAxis) ? 0 : 18, 18, 18);
        } else {
            guiObj.drawTexturedRect(guiWidth + this.guiXSize + 3, guiHeight + 36, renderX, 36, 18, 18);
        }
